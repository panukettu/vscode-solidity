...
import { ParsedCodeTypeHelper } from "./utils/ParsedCodeTypeHelper";
...
export class ParsedParameter extends ParsedVariable {
  ...
  public getSignatureInfo(paramIndex?: number, skipSelf?: boolean): string {
    if (
      ...
      this.parent instanceof ParsedFunction
    ) {
      const { paramsInfo, returnInfo } = this.parent.getParamsInfo(
        paramIndex,
        skipSelf
      );
  ...
  public getSignatureDoc(): string {
    const parentFunc =
      this.parent instanceof ParsedFunction ? this.parent : null;
    let prefix = "";
    if (parentFunc) {
      const funcPrefix = parentFunc.getParsedParentType();
      prefix = ["Contract", "Interface"].includes(funcPrefix)
        ? ""
  ...
  private getInfoText(useActive: boolean, isActive: boolean): string {
    ...
    if (this.isInput) {
      const onlyParam =
        this.parent instanceof ParsedFunction && this.parent.input.length === 1;
      const info = this.getElementInfo(useActive, isActive);
      infoText = onlyParam ? `(${info})` : `(...${info})`;
    } else if (this.isOutput) {
      const hasInput =
        this.parent instanceof ParsedFunction && this.parent.input.length > 0;
      infoText =
        (hasInput ? `(...)` : `()`) +
        `: ${this.getElementInfo(useActive, isActive)}`;
    } else {
  ...
  public getElementInfo(useActive = false, isActive = false): string {
    let id = this.element.id != null ? this.element.id : "";
    const storageType = this.getStorageType(
      (!useActive && !isActive && !!id) || (useActive && isActive && !!id)
    );
