...
export function errorToDiagnostic(error: any): CompilerError {
  if (
    ...
    error.sourceLocation.file !== null
  ) {
    const fileName = error.sourceLocation.file;

    const errorSplit = error.formattedMessage
      .substr(error.formattedMessage.indexOf(fileName))
      .split(":");
    let index = 1;
...
export function splitErrorToDiagnostic(
  error: any,
  errorSplit: any,
  index: number,
  fileName: any
  ...
  let endCharacter =
    column + error.sourceLocation.end - error.sourceLocation.start - 1;
  if (endCharacter < 0) {
    endCharacter = 1;
  }
