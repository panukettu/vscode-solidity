...
export class AddressChecksumCodeActionProvider
  implements vscode.CodeActionProvider
{
  ...
  private static regex =
    /Correct checksummed address: "0x(?<address>[0-9a-fA-F]*)"/gm;
      ...
      if (match.groups["address"]) {
        ...
        fix.edit.replace(
          ...
          new vscode.Range(
            ...
            diagnostic.range.start.translate(0, fixedAddress.length + 2)
          ),
          "0x" + fixedAddress
        );
  ...
  public provideCodeActions(
    ...
    return context.diagnostics
      .filter(
        (diagnostic) =>
          diagnostic.code ===
          AddressChecksumCodeActionProvider.ADDRESS_CHECKSUM_ERRORCODE
      )
      .map((diagnostic) =>
        AddressChecksumCodeActionProvider.createFix(document, diagnostic)
      );
  }
}

export class ChangeCompilerVersionActionProvider
  implements vscode.CodeActionProvider
{
  public static readonly providedCodeActionKinds = [
    vscode.CodeActionKind.QuickFix,
  ...
  public static createFix(
    ...
    const fix = new vscode.CodeAction(
      `Change workspace compiler version`,
      vscode.CodeActionKind.QuickFix
    );
    fix.command = {
  ...
  public provideCodeActions(
    ...
    const diagnostics = context.diagnostics.filter(
      (diagnostic) =>
        diagnostic.code ===
        ChangeCompilerVersionActionProvider.COMPILER_ERRORCODE
    );
    diagnostics.forEach((diagnostic) => {
      results.push(
        ChangeCompilerVersionActionProvider.createFix(document, diagnostic)
      );
    });
    return results;
  }
...
export class SPDXCodeActionProvider implements vscode.CodeActionProvider {
  ...
  public provideCodeActions(
    ...
    const results: vscode.CodeAction[] = [];
    const diagnostics = context.diagnostics.filter(
      (diagnostic) => diagnostic.code === SPDXCodeActionProvider.SPDX_ERRORCODE
    );
    diagnostics.forEach((diagnostic) => {
      results.push(
        SPDXCodeActionProvider.createFix(
          document,
          diagnostic,
          SPDXCodeActionProvider.preferredLicense,
          true
        )
      );
