...
export class AddressChecksumCodeActionProvider implements vscode.CodeActionProvider {
	public static readonly providedCodeActionKinds = [vscode.CodeActionKind.QuickFix, vscode.CodeActionKind.Empty];
...
export class ChangeCompilerVersionActionProvider implements vscode.CodeActionProvider {
	...
	public provideCodeActions(
		...
		const diagnostics = context.diagnostics.filter(
			(diagnostic) => diagnostic.code === ChangeCompilerVersionActionProvider.COMPILER_ERRORCODE
		);
...
export class SPDXCodeActionProvider implements vscode.CodeActionProvider {
	...
	public provideCodeActions(
		...
		for (const diagnostic of diagnostics) {
			results.push(
				SPDXCodeActionProvider.createFix(document, diagnostic, SPDXCodeActionProvider.preferredLicense, true)
			);
			for (const license of SPDXCodeActionProvider.licenses) {
