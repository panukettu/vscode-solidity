...
export class Compiler {
  ...
  private outputCompilerInfo(overrideDefaultCompiler: compilerType = null) {
    ...
    if (compiler.compilerType === compilerType.localNodeModule) {
      this.outputChannel.appendLine(
        "Compiler using solidity from node_module: " +
          compiler.getConfiguration() +
          " solidity version: " +
          compiler.getVersion()
      );
    ...
    if (compiler.compilerType === compilerType.embedded) {
      this.outputChannel.appendLine(
        "Compiler using default compiler (embedded on extension), solidity version: " +
          compiler.getVersion()
      );
    }
  }
