    ...
    return new Promise<void>((resolve, reject) => {
      ...
      if (this.canCompilerBeLoaded()) {
        try {
          ...
          this.localSolc = null;
          reject(
            "An error has ocurred, loading the compiler from Node Modules, located at:" +
              this.getLocalSolcNodeInstallation() +
              ", " +
              e
          );
        }
        ...
        this.localSolc = null;
        reject(
          "Compiler cannot be loaded from: " +
            this.getLocalSolcNodeInstallation()
        );
      }
    });
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      if (this.canCompilerBeLoaded()) {
        try {
          ...
          this.localSolc = null;
          reject(
            "An error has ocurred, loading the compiler located at:" +
              this.localPath +
              ", " +
              e
          );
        ...
        new RemoteReleases()
          ...
          .then((resolvedVersion) =>
            this.loadRemoteVersionRetry(resolvedVersion, 1, 3)
              ...
              .catch((error) => {
                reject(
                  "There was an error loading the remote version: " +
                    this.version +
                    "," +
                    error
                );
