...
export class SourceDocumentCollection {
  ...
  public static getAllLibraryImports(codeFiles: string[]): string[] {
    let imports: string[] = [];
    codeFiles.forEach(
      (x) => (imports = imports.concat(SourceDocument.getAllLibraryImports(x)))
    );
  ...
  public getDefaultSourceDocumentsForCompilation(
    optimizeCompilationRuns = 200
  ) {
    ...
    return this.getSourceDocumentsForCompilation(
      true,
      optimizeCompilationRuns,
  ...
  public addSourceDocumentAndResolveImports(
    contractPath: string,
    code: string,
    project: Project
  ) {
  ...
  private addSourceDocumentAndResolveDependencyImport(
    ...
    if (remapping !== undefined && remapping !== null) {
      const importPath = this.formatContractPath(
        remapping.resolveImport(dependencyImport)
      );
      ...
      if (depPack !== undefined) {
        const depImportPath = this.formatContractPath(
          depPack.resolveImport(dependencyImport)
        );
        this.addSourceDocumentAndResolveDependencyImportFromContractFullPath(
          depImportPath,
          project,
